      ...
      <ul>
        <li v-for="(item,index) in goods" class="menu-item" v-bind:class="{'current':currentIndex===index}"
          ...
          <span class="text border-1px"><span v-show="item.type>0" class="icon"
                                              v-bind:class="classMap[item.type]"></span>
{{item.name}}</span>
        </li>
      </ul>
    </div>
  ...
  export default {
    ...
    methods:{
      selectMenu(index,event) {
        if(!event._constructed){//prevent mobile double event
          return;
        }
      ...
      _initScroll() {
        this.menuScroll = new BScroll(this.$refs.menuWrapper, {
          click: true //get click event
        });
        this.foodsScroll = new BScroll(this.$refs.foodsWrapper, {
          click: true,
          probeType: 3  //get mouse event in real-time
        });
      ...
      _calculateHeight() {
        let foodlist = this.$refs.foodsWrapper.getElementsByClassName('food-list-hook');
        // console.log("foodlist:"+foodlist+"length:"+foodlist.length);
        let height = 0;
        this.listHeight.push(height);
        for(let i=0; i < foodlist.length; i++){
          let item = foodlist[i];
          height += item.clientHeight;
          // console.log("i:"+i+" height:"+height);
          this.listHeight.push(height);
        }
      },
    },
